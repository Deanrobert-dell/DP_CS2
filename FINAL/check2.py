...
def draw_board():
    for row in range(10):
        for col in range(10):
            color = "white" if (row + col) % 2 == 0 else "black"  # defines which square is which color
            pygame.draw.rect(screen, color, (col * 100, row * 100,   100, 100))

def draw_pieces():
    for (col, row), color in board.items():
        hex_color = "#B30000" if color == "red" else "#0600B2" #piece colors
        pygame.draw.circle(screen, hex_color, (col * 100 + 50, row * 100 + 50), 40)
...
def move_piece(start, end, captured_piece=None):
    global current_turn, message, game_over
    if start in board:
        ...
        if captured_piece:
            ...
            message = f"{color.capitalize()} jumped and captured enemy at {captured_piece}!"
        else:
...
def ai_move():
    global message, game_over #fix global later
    if game_over:
        return
    ...
    if jump_pieces:
        ...
        message = "Ai cant move, blue wins"
        game_over = True

# Game Variables
selected_piece = None
message = "blue, select a piece"
current_turn = "blue" 
game_over = False
running = True
...
while running:
    ...
    if current_turn == "red" and not game_over:
        ...
        draw_board()
        draw_pieces()
        text_surf = font.render(message, True, "#00B115")
        ...
        if current_turn == "blue" and not game_over and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if selected_piece is None:
                if board.get(clicked_square) == "blue":
                    ...
                    message = f"selction on  {selected_piece}"
                else:
                    message = "invalid"
            else:
